...
export default function ProjectForm() {
  const [progress, setProgress] = useState(50); // valor inicial do slider
  const [images, setImages] = useState<string[]>([]); // preview URLs (em produção: File[])
  const [isSubmitting, setIsSubmitting] = useState(false);

  const isEditMode = false; // Em produção: viria da rota ou contexto (true = editar)
  ...
  const {
    ...
    defaultValues: {
      title: isEditMode ? 'Jogo de Zungueira' : '',
      description: isEditMode ? 'Jogo mobile educativo sobre a cultura das zungueiras...' : '',
      progress: isEditMode ? 50 : 0,
    },
  });

  const onSubmit = async (data: FormData) => {
    setIsSubmitting(true);
    console.log('Dados do projeto:', { ...data, progress, images: images.length });
    // Simulação de envio
    await new Promise(resolve => setTimeout(resolve, 2000));
    setIsSubmitting(false);
    alert(isEditMode ? 'Projeto atualizado com sucesso!' : 'Projeto cadastrado com sucesso!');
  };
        ...
        <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-3xl p-8 md:p-10 shadow-2xl">
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
            ...
            <div className="space-y-3">
              ...
              <textarea
                rows={6}
                placeholder="Descreva o objetivo, tecnologias usadas, desafios e o que já foi desenvolvido..."
                className={`w-full px-5 py-4 bg-white/10 border ${
            ...
            <div className="space-y-4">
              <label className="text-lg font-semibold text-white">
                Capturas de Tela / Imagens do Projeto (opcional)
              </label>
